...
from agents import MODELS, DEFAULT_MODEL
from github_fetch import fetch_code
from github_auth import (
    oauth_configured, get_auth_url, get_redirect_uri,
...
MODEL_API_KEYS = {
    "claude-sonnet-4-6": ("ANTHROPIC_API_KEY", "Anthropic API Key", "sk-ant-..."),
    "gpt-4o":            ("OPENAI_API_KEY",     "OpenAI API Key",    "sk-..."),
    "gemini-2.0-flash":  ("GEMINI_API_KEY",     "Gemini API Key",    "AIza..."),
}
...
# ── Sidebar: model + API key only ────────────────────────────────────────────
with st.sidebar:
    st.subheader("Model")
    model_label = st.selectbox("Reviewer model", list(MODELS.keys()), index=0)
    model_id    = MODELS[model_label]

    env_var, key_label, key_placeholder = MODEL_API_KEYS[model_id]
...
with left:
    ...
    if github_token:
        ...
        if oauth_configured():
            ...
            st.info(
                "**For public repos** — no connection needed, enter URL below.\n\n"
    ...
    if not api_key:
        st.warning(f"Add your {key_label} in the sidebar.")
    elif not code:
...
if run and code.strip():
    with right:
                    ...
                    if node_name == "agents":
                        for agent_name, meta in AGENT_META.items():
                            ...
                            placeholders[agent_name].markdown(
                                ...
                                f"{count} finding{'s' if count != 1 else ''} found"
                            )
        ...
        # Store results so they persist on rerender
...
# ── Render stored results ─────────────────────────────────────────────────────
    ...
    with right:
        ...
        if not findings:
            ...
            st.subheader("What the Agents Found")
            m1, m2, m3 = st.columns(3)
            m1.metric("Total Issues", len(findings))
            m2.metric("🔴 Must Fix", len(fix_items), help="2+ agents agreed")
            m3.metric("🟡 Your Call", len(call_items), help="Only 1 agent flagged")
            ...
            with st.expander("Per-agent breakdown"):
                for key, meta in AGENT_META.items():
                    ...
                    st.markdown(f"{meta['icon']} **{meta['label']}** — {len(items)} finding{'s' if len(items) != 1 else ''}")
                    for item in items:
            ...
            if fix_items:
                ...
                st.success("No must-fix issues.")

            if call_items:
                st.subheader("Your Call")
                st.caption("Flagged by one reviewer — genuine tradeoffs, not clear bugs.")
                for f in call_items:
                    agents = f.get("agents", [])
                    icons  = " ".join(AGENT_META[a]["icon"] for a in agents if a in AGENT_META)
                    with st.container(border=True):
